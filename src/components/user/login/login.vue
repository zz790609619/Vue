		...
		methods: {
			...
			login () {
				...
				fecth.post(API.LOGIN, {
					...
					if (res.data.code === '1') {
						Storage.setCookie('c_user_info', JSON.stringify(res.data.entity), 60 * 60 * 1000 * 24)
						this.$store.dispatch('set_UserInfo', res.data.entity)
			...
			singin () {
				...
				fecth.post(API.SINGIN, {
					...
					this.$msg(res.data.msg)
					if (res.data.code === '1') {
						console.log(res.data.entity.username)
						this.rightSigninName = res.data.entity.username
						// 显示后续的操作
						this.singinThen()
					}
				}, (err) => {
